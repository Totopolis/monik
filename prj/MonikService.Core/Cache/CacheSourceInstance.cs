...
namespace MonikService.Core.Cache
{
    public class SourceInstanceCache : ISourceInstanceCache
    {
        ...
        public void OnStart()
        {
            ...
            foreach (var ins in instances)
                if (_sourceMap.ContainsKey(ins.SourceID))
                {
                    ...
                    _control.ApplicationError(
                        $"Database doesnt contains source(id={ins.SourceID}) for the instance '{ins.Name}'");
        ...
        {
            if (!_groups.ContainsKey(aGroupId)) return false;
        ...
        public Instance CheckSourceAndInstance(string aSourceName, string aInstanceName)
        {
            ...
            lock (this)
            {
                if (_sourceInstanceMap.ContainsKey(key)) return _sourceInstanceMap[key];
