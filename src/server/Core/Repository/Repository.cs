...
namespace MonikService.Core.Repository
{
    public class Repository : IRepository
    {
        ...
        {
            return SimpleCommand.ExecuteQuery<Source>(_settings.DbConnectionString, "select * from [mon].[Source]")
                                .ToList();
        }

        public List<Instance> GetAllInstances()
        {
            return SimpleCommand.ExecuteQuery<Instance>(_settings.DbConnectionString, "select * from [mon].[Instance]")
                                .ToList();
        }
        ...
        public List<Group> GetAllGroupsAndFill()
        {
            ...
            foreach (var it in result) dic.Add(it.ID, it);
        ...
        {
            aSrc.ID = (short) MappedCommand.InsertAndGetId<Source>(_settings.DbConnectionString, "[mon].[Source]", aSrc,
                                                                   "ID");
        }

        public void CreateNewInstance(Instance aIns)
        {
            aIns.ID = (int) MappedCommand.InsertAndGetId<Instance>(_settings.DbConnectionString, "[mon].[Instance]",
                                                                   aIns, "ID");
        }
        ...
        public List<Log_> GetLastLogs(int aTop)
        {
            return
                SimpleCommand.ExecuteQuery<Log_>(_settings.DbConnectionString,
                                                 $"select top {aTop} * from [mon].[Log] order by ID desc")
                             .OrderBy(x => x.ID)
                             .ToList();
        }
        ...
        public long? GetLogThreshold(int aDayDeep)
        {
            var logThreshold = SimpleCommand.ExecuteScalar(_settings.DbConnectionString,
                                                           "select max(LastLogID) from mon.HourStat where Hour < DATEADD(DAY, -@p0, GETDATE())",
                                                           aDayDeep);
        ...
        {
            return SimpleCommand.ExecuteNonQuery(_settings.DbConnectionString, "delete from mon.Log where ID < @p0",
                                                 aLastLog);
        }

        public void CreateLog(Log_ aLog)
        {
            aLog.ID = (long) MappedCommand.InsertAndGetId<Log_>(_settings.DbConnectionString, "[mon].[Log]", aLog,
                                                                "ID");
        }
        ...
        {
            var result =
                SimpleCommand.ExecuteScalar(_settings.DbConnectionString, "select max(ID) from [mon].[KeepAlive]");
        ...
        public List<KeepAlive_> GetLastKeepAlive(int aTop)
        {
            return
                SimpleCommand.ExecuteQuery<KeepAlive_>(_settings.DbConnectionString,
                                                       $"select top {aTop} * from [mon].[KeepAlive] order by ID desc")
                             .ToList();
        }
        ...
        public long? GetKeepAliveThreshold(int aDayDeep)
        {
            var kaThreshold = SimpleCommand.ExecuteScalar(_settings.DbConnectionString,
                                                          "select max(LastKeepAliveID) from mon.HourStat where Hour < DATEADD(DAY, -@p0, GETDATE())",
                                                          aDayDeep);
        ...
        public int CleanUpKeepAlive(long aLastKeepAlive)
        {
            return SimpleCommand.ExecuteNonQuery(_settings.DbConnectionString,
                                                 "delete from mon.KeepAlive where ID < @p0",
                                                 aLastKeepAlive);
        }
        ...
        {
            aKeepAlive.ID =
                (long) MappedCommand.InsertAndGetId<KeepAlive_>(_settings.DbConnectionString, "[mon].[KeepAlive]",
                                                                aKeepAlive, "ID");
        }
        ...
        {
            return
                SimpleCommand.ExecuteQuery<EventQueue>(_settings.DbConnectionString, "select * from [mon].[EventQueue]")
                             .ToList();
        }
        ...
        public List<MetricDescription> GetMetricDescriptions()
        {
            return SimpleCommand
                  .ExecuteQuery<MetricDescription>(_settings.DbConnectionString,
                                                   $"select * from [mon].[{nameof(MetricDescription)}]")
                  .ToList();
        }
        ...
        {
            return SimpleCommand
                  .ExecuteQuery<MetricValue>(_settings.DbConnectionString,
                                             $"select * from [mon].{nameof(MetricValue)} where MetricId = {metricId}")
                  .ToList();
        }

        public void CreateMetricDescription(MetricDescription metricDescription)
        {
            metricDescription.Id = (int) MappedCommand.InsertAndGetId<MetricDescription>(
                _settings.DbConnectionString, $"[mon].[{nameof(MetricDescription)}]", metricDescription, "Id");
        }
